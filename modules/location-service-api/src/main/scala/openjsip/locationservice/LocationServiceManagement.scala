...
trait LocationServiceManagement {
  def registerSubscriber(addressOfRecord: String): LocationServiceKey

  def unregisterSubscriber(addressOfRecord: String)

  def setBindingsExpirationCheckPeriod(seconds: Int)
}
